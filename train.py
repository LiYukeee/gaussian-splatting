    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                logger.info("Num of gaussians: {} K".format(float(scene.gaussians.get_xyz.shape[0]) / 1000))
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                logger.info("======== [ITER {}] Evaluating {} ========".format(iteration, config['name']))
                logger.info("L1 {} PSNR {}".format(l1_test, psnr_test))
                logger.info("Num of gaussians: {} K".format(float(scene.gaussians.get_xyz.shape[0]) / 1000))
                if tb_writer:
