...
def generate_path(viewpoint_cameras, n_frames=480):
  ...
  for c2w in new_poses:
      ...
      cam.world_view_transform_opengl = cam.world_view_transform.detach().clone()
      cam.world_view_transform_opengl[:, 0] = -1 * cam.world_view_transform_opengl[:, 0]
      cam.world_view_transform_opengl[:, 1] = -1 * cam.world_view_transform_opengl[:, 1]

      cam.projection_matrix_opengl = cam.projection_matrix.detach().clone()
      cam.projection_matrix_opengl[2, 2] = 0.0
      cam.projection_matrix_opengl[3, 2] = 1.0
      cam.projection_matrix_opengl[2, 3] = 1.0
      cam.full_proj_transform_opengl = (cam.world_view_transform_opengl.unsqueeze(0).bmm(cam.projection_matrix_opengl.unsqueeze(0))).squeeze(0)
      traj.append(cam)
